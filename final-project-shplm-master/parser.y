...
%type  <exp_nodes> expression noterminal logical conditional
%type  <exp_nodes> program statements statement id
%type  <exp_nodes> elseconditional ifstatement whilestatement breakstatement
...
program
     : statements{$$ = new node; $$ = $1;  $$->label = "Block"; nodes = $$;nodes->id = "n0";}
     ;
statements
     : statement  {$$ = new node;
...
id:IDENTIFIER               {         $$ = new node;
                                      ...
                                      ;
breakstatement
              : BREAK NEWLINE {$$ = new node;$$->label = "break"; }
              ;
expression
          : noterminal                        { $$ = new node;$$ = $1; }
          ...
          ;
noterminal
          : IDENTIFIER               {$$ = new node;$$->label="Identifier "; $$->shape = "box"; $$->str = *$1;
                                      $$->label = $$->label + *$1;}
          ...
          ;
ifstatement
          :IF logical COLON NEWLINE conditional elseconditional {
                          $$ = new node;
           ...
           ;
whilestatement
               :WHILE logical COLON NEWLINE conditional DEDENT {
                                                                $$ = new node;
                                                                $$->next_node.push_back($2);
                                                                $$->label = "While";
                                                                $$->next_node.push_back($5); }
              ;
conditional
           : INDENT statements {$$ = new node;$$=$2; $$->label="Block"; }
           ;
elseconditional
               : DEDENT {$$= new node;  $$->label = "NULL";}
